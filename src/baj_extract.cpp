  ...
  if (barcorrect){
    ...
    // Iterate over each generated barcode to find mismatch positions
      ...
      if (min_hamming_results.size() == 1) {
        ...
        if (best_matches.size() > 1) {
            nwl_ids[i] += "{jaccard_multi_" + nwl_barcodes[i] + "_" + std::to_string(best_matches.size()) + ":jac_" + std::to_string(min_jaccard_distance) + ":ham_" + std::to_string(min_hamming_results.size()) + "}";
