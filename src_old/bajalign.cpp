...
// [[Rcpp::export]]
Rcpp::DataFrame bajalign_stats(Rcpp::CharacterVector adapters, std::vector<std::string> sequences, int nthreads = 1) {
...
// [[Rcpp::export]]
Rcpp::CharacterVector bajalign_sigs(Rcpp::CharacterVector adapters, std::vector<std::string> sequences, 
  Rcpp::DataFrame null_distance, int nthreads) {
  std::vector<std::string> queries = Rcpp::as<std::vector<std::string>>(adapters);
  ...
  Rcpp::StringVector query_ids = null_distance["query_id"];
  Rcpp::NumericVector null_dists = null_distance["null_distance"];
  Rcpp::NumericVector sd_nulls = null_distance["sd_null"];
  for(int i = 0; i < query_ids.size(); ++i) {
    null_dist_map[Rcpp::as<std::string>(query_ids[i])] = std::make_pair((double) null_dists[i], (double) sd_nulls[i]);  
  }
  omp_set_num_threads(nthreads);
...
#pragma omp parallel for
    for (int j = 0; j < queries.size(); ++j) {
      ...
      if (query_name == "poly_a" || query_name == "poly_t") {
        ...
        std::string match_str = findPolyTail(sequence, poly_base, 16, 12);  // assuming window_size = 12 and min_count = 10
        if (!match_str.empty()){
#pragma omp critical
{
